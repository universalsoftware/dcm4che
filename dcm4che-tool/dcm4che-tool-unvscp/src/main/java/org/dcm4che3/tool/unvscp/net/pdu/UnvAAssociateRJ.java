...
package org.dcm4che3.tool.unvscp.net.pdu;
 ...
 */
public class UnvAAssociateRJ extends AAssociateRJ{
    ...
    public static final int REASON_USER_DEFINED = 0;
    ...
    public UnvAAssociateRJ(int result, int source, int reason, String reasonText) {
        super(result, source, reason);
        this.result = result;
        this.source = source;
        this.reason = reason;
        if (source == UnvAAssociateRJ.SOURCE_SERVICE_USER) {
            UnvAAssociateRJ.SERVICE_USER_REASONS[reason] = reason + " - " + reasonText;
        }
    }
    ...
    public String getMessage() {
        return "A-ASSOCIATE-RJ[result: " + toString(RESULTS, result)
